...
export const getHealth = async (): Promise<{
  ...
  try {
    const response: { success: string; data: string } =
...
export const createWallet = async (): Promise<CreateWalletResponse> => {
  try {
    const response: AxiosResponse<CreateWalletResponse> =
      await AxiosInstance.post("/api/wallets", {});
...
export const getWalletAddresses = async (
  ...
  try {
    const response = await AxiosInstance.get("/api/wallets/addresses", {
...
export const getAddressByIndex = async (
  ...
  try {
    const response: AxiosResponse<Wallet> = await AxiosInstance.get(
      `/api/wallets/addresses/by-index?index=${index}`,
      { headers: { Authorization: `Bearer ${credentials}` } },
    );
    return response.data;
  } catch (error: any) {
